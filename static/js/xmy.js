...
var xmy = {
    ...
    getData: function(json){
        var token = this.getQueryString('token'),
            ...
            merchantId = this.getQueryString('merchantId');
        return $.ajax({
            url: json.url + '?t=' + new Date().getTime() + '&token=' + token + '&userId=' + userId + '&merchantId=' + merchantId,
            type: json.type || 'GET',
            data: json.data || '',
            ...
            error: function (xhr,textStatus,errorThrown) {
                if (xhr.status == 401) {
                    window.location.href = '/app-huopan/login';
                }
            }
        });
    }
